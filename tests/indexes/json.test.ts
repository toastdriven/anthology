...
describe("JSONIndex", () => {
  ...
  describe("getTerm", () => {
    ...
    test("returns vectors for a known term", () => {
      const index = new JSONIndex(TMP_DIR);
      index.addTerm({ term: "hello", vector: { id: "1", location: 0 } });
  ...
  describe("addTerm", () => {
    ...
    test("does not add exact duplicates", () => {
      const index = new JSONIndex(TMP_DIR);
      const tv: TermVector = { term: "cat", vector: { id: "1", location: 0 } };
      index.addTerm(tv);
      index.addTerm(tv);
  ...
  describe("deleteDocument", () => {
    test("removes all vectors for a docId across all terms", () => {
      const index = new JSONIndex(TMP_DIR);
      index.addTerm({ term: "cat", vector: { id: "1", location: 0 } });
  ...
  describe("serialize", () => {
    test("returns a plain object (JSON-safe)", () => {
      const index = new JSONIndex(TMP_DIR);
      index.addTerm({ term: "cat", vector: { id: "1", location: 0 } });
      index.addTerm({ term: "dog", vector: { id: "2", location: 4 } });
    ...
    test("round-trips through JSON.stringify without data loss", () => {
      ...
      expect(JSON.parse(json)).toEqual({ cat: [{ id: "1", location: 0 }] });
    });
  ...
  describe("deserialize", () => {
    test("reconstructs data from a plain object", () => {
      const index = new JSONIndex(TMP_DIR);
      const raw = { cat: [{ id: "1", location: 0 }] };
      const map = index.deserialize(raw);
      expect(map.get("cat")).toEqual([{ id: "1", location: 0 }]);
    });
  ...
  describe("save + load", () => {
    test("load is a no-op when the file does not exist", async () => {
      ...
      await index.load();
      expect(index.getTerm("anything")).toEqual([]);
    });
    ...
    test("save writes a valid JSON file to disk", async () => {
      ...
      expect(parsed).toEqual({ cat: [{ id: "1", location: 0 }] });
    });
    ...
    test("load restores data saved by save", async () => {
      ...
      writer.addTerm({ term: "cat", vector: { id: "1", location: 0 } });
      writer.addTerm({ term: "dog", vector: { id: "2", location: 4 } });
      await writer.save();

      const reader = new JSONIndex(TMP_DIR);
      await reader.load();
      expect(reader.getTerm("cat")).toEqual([{ id: "1", location: 0 }]);
      expect(reader.getTerm("dog")).toEqual([{ id: "2", location: 4 }]);
    });
